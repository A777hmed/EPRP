...
export function WeeklyReportFormView({ reportId }: WeeklyReportFormViewProps) {
  ...
  const [loaded, setLoaded] = React.useState<{
    ...
    entries: WeeklyEntry[];
  } | null>(isEdit ? null : { report: null, submissions: [], entries: [] });
  ...
  React.useEffect(() => {
    ...
    weeklyReportService.getById(reportId).then(async (found) => {
      if (!found) {
        setLoaded({ report: null, submissions: [], entries: [] });
        return;
      }
      ...
      setLoaded({ report: found, submissions, entries });
    });
  }, [reportId]);
  ...
  const { report, submissions, entries } = loaded;
  ...
  const initialValues = report
    ? weeklyReportToHeaderValues(report, submissions, entries)
    : emptyWeeklyReportHeaderValues();
