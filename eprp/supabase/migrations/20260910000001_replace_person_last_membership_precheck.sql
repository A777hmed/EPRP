-- EPRP — Replace Person, Pass A correction 2: last-membership orphan guard.
--
-- FOUND DURING LOCAL SMOKE TESTING (20260909), flagged for a decision rather
-- than fixed immediately because it never fired in the required test matrix.
-- Closed now, before Pass B UI work begins.
--
-- THE EDGE CASE
--   public.project_contacts carries `guard_project_contact_membership_removal()`
--   (pre-existing, not part of Replace Person): a BEFORE UPDATE/DELETE trigger
--   that refuses to remove or reassign a contact's LAST project_contacts row on
--   a project while that contact is still referenced, on the SAME project, by
--   a fixed responsibility column on public.projects or by a
--   public.project_positions row. It exists so a fixed responsibility or a
--   named position can never end up pointing at someone with no team
--   membership left to anchor it.
--
--   The department_assignment branch of replace_project_responsibility()
--   (20260909000001) never anticipated this. If the outgoing person's ONLY
--   project_contacts row(s) on the project are exactly the ones this call
--   retargets, and they are still a fixed responsibility holder or a
--   project_positions holder on that same project, the retargeting UPDATE
--   reaches the trigger and it raises its own message — not wrapped in this
--   function's [TAG] convention, so the application's tag parser falls back to
--   a fully generic refusal instead of a specific, actionable one. The
--   trigger's refusal itself was always correct; only the message reaching the
--   caller was not.
--
-- FIX — additive, `create or replace function` only, mirrors the trigger's
-- own predicate exactly (same two references, same project scope) so the
-- refusal happens with this function's own message before the trigger is ever
-- reached. Checked only for department_assignment, because that is the only
-- shape whose write can ever remove a contact's last project_contacts row —
-- fixed_responsibility and project_position writes never delete a
-- project_contacts row at all.
--
-- WHAT THIS DOES NOT DO
--   Does not remove or reassign the outgoing person's other responsibility.
--   Does not fabricate a membership row to work around the trigger. Does not
--   replace the person anywhere else. Does not touch any other project. Does
--   not alter, relax, or bypass guard_project_contact_membership_removal()
--   itself — this is a read-only pre-check that mirrors it so the refusal is
...
begin
  ...
  /* ------------------------- Shape 2: department assignment --------------- */
    ...
    -- NEW (this migration): would this retarget leave the outgoing person with
    -- ZERO project_contacts rows on this project, while a fixed responsibility
    -- column or a project_positions row here still names them? Mirrors
    -- guard_project_contact_membership_removal()'s own predicate exactly —
    -- same two references, same project scope — so that trigger's refusal
    -- (which fires regardless, unchanged) is anticipated here with a semantic
    -- message instead of reaching the caller as a raw one. This does not
    -- reassign or remove the other responsibility, does not touch any other
    -- project, and does not weaken the trigger itself.
    select count(*) into v_remaining_membership_count
      from public.project_contacts
     where project_id = p_project
       and contact_id = p_from_contact
       and not (
    ...
    if v_remaining_membership_count = 0 then
      select
        ...
        or exists (
          select 1 from public.project_positions pp
           where pp.project_id = p_project and pp.contact_id = p_from_contact
        )
        into v_still_referenced;

      if v_still_referenced then
        raise exception '[LAST_MEMBERSHIP_REFERENCED] This person still holds another project responsibility. Reassign that responsibility first before removing their final project team assignment.'
          using errcode = 'foreign_key_violation';
      end if;
    end if;
...
comment on function public.replace_project_responsibility(uuid, text, uuid, uuid, text, uuid, text, uuid, text) is
  'Replace Person, Pass A (corrected 20260909000002 for project-membership pre-validation; corrected 20260910000001 to pre-validate that a department_assignment retarget will not orphan a fixed responsibility or project_positions reference by removing the outgoing persons last project_contacts row). Swaps who holds ONE identified project responsibility and writes one project_responsibility_history row, atomically. Authorized by can_manage_project_responsibilities() — System Admin / Project Control Admin (global) or the projects own assigned Project Control / Planning only. Never touches Auth, profiles, Users & Roles, other projects, or historical report attribution.';
...
begin
  ...
  if not exists (
    select 1 from pg_trigger
     where tgrelid = 'public.project_contacts'::regclass
       and tgname = 'trg_project_contact_membership_removal'
  ) then
    -- Not fatal to this migration — this is a warning that the mirrored
    -- predicate above may have drifted from what actually still guards the
    -- table, named so it is never mistaken for silent success.
    raise warning 'guard_project_contact_membership_removal trigger not found by that name — verify the pre-check above still mirrors the live guard.';
  end if;

  raise notice 'Replace Person last-membership pre-check installed: department_assignment now refuses to orphan a fixed responsibility or project_positions reference with [LAST_MEMBERSHIP_REFERENCED] before writing, instead of surfacing the trigger''s raw exception.';
end;
$post$;
