...
|---|---|---|---|
| 1 | Report status values | `weekly_reports.status` has **no CHECK constraint**; code uses a 10-state lifecycle (`config/workflows.ts`) | The 6-state workflow in §3 (+ returned/rejected/archived handling per `03_WORKFLOW.md`). Reconcile `config/workflows.ts` and add a constraint in a new migration |
